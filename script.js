window.addEventListener('load', function() {
    ...
    class InputHandler{
        constructor(game){
            ...
            window.addEventListener('keydown', e => {
                ...
                && (this.game.keys.indexOf(e.key) === -1) && !this.game.gameOver) 
                {
    ...
    class Projectile{
        ...
        draw(context){
            context.drawImage(this.image, 583, 92, this.width, this.height, this.x, this.y, this.width/2, this.height/2);
        }
    }
    class Particle{
    ...
    class Player{
        ...
        update(){ 
            if (this.game.keys.includes('ArrowUp') && this.y >= this.game.height * 0.38) this.speedY = -this.maxSpeed;
            else if (this.game.keys.includes('ArrowDown') && this.y <= this.game.height - this.height) this.speedY = this.maxSpeed;
            else this.speedY = 0;
            this.y += this.speedY; 
            // handle projectiles
        ...
        shootTop(){
            if (this.game.ammo > 0){
            this.projectiles.push(new Projectile(this.game, this.x + this.width, this.y + this.height/1.6));
            this.game.ammo--;
            }
        }
    }
    class Enemy{
        ...
        draw(context){
            if (this.game.debug) context.strokeRect(this.x, this.y, this.width, this.height);
            if (this.type == "lucky" || this.type == "arrowB") context.drawImage(this.image, this.x, this.y, this.width, this.height);
            ...
            if (this.game.debug) context.fillText(this.lives, this.x + this.width/2, this.y + this.height/2);
        }
    }
    class Mage extends Enemy{
        constructor(game){
            ...
            this.y = (this.game.height * 0.38) + Math.random() * ((this.game.height * 0.5) - this.height);
            this.frameX = 0;
            this.frameY = 0;
            this.animate = false;
    ...
    class UI{
        ...
        draw(context){
            ...
            // time
            context.fillText('Time: ' + (this.game.gameTime/1000).toFixed(1) + 's', this.game.width - 150, 30);
            // score
            context.fillText('Score: ' + this.game.score, 20, 30);
            // ammo
            context.fillStyle = this.color;
            for (let i = 0; i < this.game.ammo; i++){
                context.drawImage(document.getElementById("arrow"), 20 + 12 * i, 50, 10, 20);
            }
            // hp
            context.fillStyle = "red";
            for (let i = 0; i < this.game.hp; i++){
                context.drawImage(document.getElementById("heart"), (this.game.width-25) - 21 * i, 60, 20, 20);
            }
            // game over message
            if (this.game.gameOver)
            {
                ...
                {
                    message1 = 'You Win!';
                    message2 = 'Well done! Your High Score is: ' + (this.game.gameTime/1000).toFixed(1) + 's';
                } else {
                    message1 = 'You lose!';
    ...
    class Game{
        ...
        addEnemy(){
            ...
            if (this.score >= 50 && randomise > 0.6)  this.enemies.push(new Mage(this));
